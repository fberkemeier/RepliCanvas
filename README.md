...
Where supported, RepliSketch asks for the MP4 filename and destination before encoding begins and verifies the completed file write when the browser exposes that check. Browsers without the File System Access API reserve a user-authorised download tab before encoding. When the animation is ready, RepliSketch presents a real **Download MP4** link that the user activates so the file is registered by the browser download manager. The same trusted-click download remains available after a file has been written to a selected location. A current Chromium-based browser provides the most reliable MP4 encoding experience.
